...
class ParseAddUniqueOperation implements ParseFieldOperation {
    ...
    public JSONObject encode(ParseObjectEncodingStrategy objectEncoder) throws JSONException {
        ...
        output.put("objects", Parse.encode(new ArrayList(this.objects), objectEncoder));
        return output;
    }
    ...
    public ParseFieldOperation mergeWithPrevious(ParseFieldOperation previous) {
        ...
        if (previous instanceof ParseSetOperation) {
            ...
            if ((value instanceof JSONArray) || (value instanceof List)) {
                return new ParseSetOperation(apply(value, null, null));
            }
            throw new IllegalArgumentException("You can only add an item to a List or JSONArray.");
        } else if (previous instanceof ParseAddUniqueOperation) {
            return new ParseAddUniqueOperation((Collection<?>) (List) apply(new ArrayList<>(((ParseAddUniqueOperation) previous).objects), null, null));
        } else {
        ...
        if (oldValue instanceof List) {
            ...
            while (i$.hasNext()) {
                ...
                if (obj instanceof ParseObject) {
                    Integer index = (Integer) existingObjectIds.get(((ParseObject) obj).getObjectId());
                    if (index != null) {
