...
public class ParseQueryAdapter<T extends ParseObject> extends BaseAdapter {
    ...
    /* access modifiers changed from: private */
    public List<List<T>> objectPages;
    ...
    public ParseQueryAdapter(Context context2, final String className) {
        this(context2, (QueryFactory<T>) new QueryFactory<T>() {
    ...
    public ParseQueryAdapter(Context context2, Class<? extends ParseObject> clazz, int itemViewResource) {
        this(context2, ParseObject.getClassName(clazz), itemViewResource);
    }

    public ParseQueryAdapter(Context context2, final String className, int itemViewResource) {
        this(context2, (QueryFactory<T>) new QueryFactory<T>() {
            public ParseQuery<T> create() {
                ParseQuery<T> query = ParseQuery.getQuery(className);
                query.orderByDescending("createdAt");
    ...
    public ParseQueryAdapter(Context context2, QueryFactory<T> queryFactory2) {
        this(context2, queryFactory2, (Integer) null);
    }

    public ParseQueryAdapter(Context context2, QueryFactory<T> queryFactory2, int itemViewResource) {
        this(context2, queryFactory2, Integer.valueOf(itemViewResource));
    }

    private ParseQueryAdapter(Context context2, QueryFactory<T> queryFactory2, Integer itemViewResource) {
    ...
    private void loadObjects(int page, boolean shouldClear) {
        ...
        final Capture<Boolean> firstCallBack = new Capture<>(Boolean.valueOf(true));
        final boolean z = shouldClear;
        final int i = page;
        query.findInBackground(new FindCallback<T>() {
            @SuppressLint({"ShowToast"})
            public void done(List<T> foundObjects, ParseException e) {
                boolean z;
                if (query.getCachePolicy() != CachePolicy.CACHE_ONLY || e == null || e.getCode() != 120) {
                    if (e != null && (e.getCode() == 100 || e.getCode() != 120)) {
                        ParseQueryAdapter.this.hasNextPage = true;
                    } else if (foundObjects != null) {
                        if (z && ((Boolean) firstCallBack.get()).booleanValue()) {
                        ...
                        List<T> currentPage = (List) ParseQueryAdapter.this.objectPages.get(i);
                        currentPage.clear();
    ...
    public void syncObjectsWithPages() {
        this.objects.clear();
        for (List<T> pageOfObjects : this.objectPages) {
    ...
    private View getDefaultView(Context context2) {
        if (this.itemResourceId != null) {
            return View.inflate(context2, this.itemResourceId.intValue(), null);
        }
