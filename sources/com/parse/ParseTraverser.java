...
abstract class ParseTraverser {
    ...
    private void traverseInternal(Object root, boolean yieldRoot2, IdentityHashMap<Object, Object> seen) {
        if (root != null && !seen.containsKey(root)) {
            if (!yieldRoot2 || visit(root)) {
                seen.put(root, root);
                ...
                } else if ((root instanceof ParseACL) && ((ParseACL) root).hasUnresolvedUser()) {
                    traverseInternal(ParseUser.getCurrentUser(), true, seen);
                }
            }
        }
    }
