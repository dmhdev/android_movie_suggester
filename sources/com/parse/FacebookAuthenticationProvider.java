...
class FacebookAuthenticationProvider implements ParseAuthenticationProvider {
    ...
    public ParseAuthenticationCallback currentOperationCallback;
    ...
    @Deprecated
    public synchronized void extendAccessToken(Context context, ParseAuthenticationCallback callback) {
        if (this.currentOperationCallback != null) {
            cancel();
        }
        this.currentOperationCallback = callback;
        if (!this.facebook.extendAccessToken(context, new ServiceListener() {
    ...
    public synchronized void authenticate(ParseAuthenticationCallback callback) {
        if (this.currentOperationCallback != null) {
            cancel();
        }
        this.currentOperationCallback = callback;
        ...
        this.session = new Builder(activity).setApplicationId(this.applicationId).setTokenCachingStrategy(new SharedPreferencesTokenCachingStrategy(activity)).build();
    ...
    public boolean restoreAuthentication(JSONObject authData) {
        ...
        try {
            ...
            tcs.save(data);
            Session newSession = new Builder(this.applicationContext).setApplicationId(this.applicationId).setTokenCachingStrategy(tcs).build();
